...
const auto stoppingCondition = [](const haste::app::Tracker &tracker) -> bool {
  return !((tracker.x() >= kRemovalMargin) && (tracker.y() >= kRemovalMargin) &&
           ((tracker.x() + kRemovalMargin) < camera.width) &&
           ((tracker.y() + kRemovalMargin) < camera.height));
};
...
int distribution_cnt = 0, distribution_cnt_addseed = 0,
...
uint16_t ReadSeed(std::string seed_data) {
  ...
  position_id_read_seed =
      static_cast<uint16_t>(y / subimage_size) *
          static_cast<uint16_t>(camera.width / subimage_size) +
      static_cast<uint16_t>(x / subimage_size);
  ...
  for (const auto &e : event_msg->events) {
    ...
    for (int itr_haste = 0; itr_haste < itr_haste_size; itr_haste++) {
      ...
      if (result == haste::app::Tracker::EventUpdate::kStateEvent) {
        ...
        position_id_tmp =
            static_cast<uint16_t>(tracker->y() / subimage_size) *
                static_cast<uint16_t>(camera.width / subimage_size) +
            static_cast<uint16_t>(tracker->x() / subimage_size);
        if (position_id_old != position_id_tmp) {
          ...
          for (int itr_distr = 0; itr_distr < max_duplication; itr_distr++) {
            if (tracker_distribution[position_id_old][itr_distr] ==
          ...
          for (int itr_distr = 0; itr_distr < max_duplication; itr_distr++) {
            if (tracker_distribution[position_id_tmp][itr_distr] == 0) {
              tracker_distribution[position_id_tmp][itr_distr] =
                  haste_trackers[itr_haste].tracker_id;
              break;
            }
          }
          // Check hypothesis score and evaluate if tracking fails
          bad_tracking_threshold = (tracker->max_hypothesis_score() -
                                    tracker->min_hypothesis_score()) /
                                   tracker->max_hypothesis_score();
          if (bad_tracking_threshold < 0.1) {
            ROS_INFO("Tracking failure... Tracker ID: %d, threshold: %f",
                     haste_trackers[itr_haste].tracker_id,
                     bad_tracking_threshold);
        ...
        if (stoppingCondition(*tracker)) {
          ...
          for (int itr_distr = 0; itr_distr < max_duplication; itr_distr++) {
            if (tracker_distribution[position_id][itr_distr] ==
                haste_trackers[itr_haste].tracker_id) {
    ...
    if (poschange_flg) {
      for (int itr_dist = 0; itr_dist < num_distribution; itr_dist++) {
        distribution_cnt = 0;
        for (int itr_dist_subimg = 0; itr_dist_subimg < max_duplication;
             itr_dist_subimg++) {
        ...
        if (distribution_cnt > 1) {
          ...
          for (int itr_dist_subimg = 0; itr_dist_subimg < max_duplication;
            ...
            if (tracker_distribution[itr_dist][itr_dist_subimg] != 0) {
              // Delete tracker
              for (int itr_haste = 0; itr_haste < haste_trackers.size();
                   itr_haste++) {
                if (haste_trackers[itr_haste].tracker_id ==
                    tracker_distribution[itr_dist][itr_dist_subimg]) {
                  // Highest alignment score
                  if (best_tracker == "alignment_score" ||
                      (best_tracker != "lifespan" &&
                       best_tracker != "hybrid")) {
                    if (max_hypothesis_score != 0 &&
                        max_hypothesis_score > haste_trackers[itr_haste]
                                                   .tracker.returnTracker()
                                                   ->max_hypothesis_score()) {
                      ...
                      break;
                    } else if (max_hypothesis_score != 0 &&
                               haste_trackers[itr_haste]
                                       .tracker.returnTracker()
                                       ->max_hypothesis_score() >=
                                   max_hypothesis_score) {
                      // Erase old haste tracker
                      for (int itr_haste_j = 0;
                           itr_haste_j < haste_trackers.size(); itr_haste_j++) {
                        if (haste_trackers[itr_haste_j].tracker_id ==
                            tracker_distribution
                                [itr_dist]
                                [max_hypothesis_score_itr_dist_subimg]) {
                      ...
                      // Delete distribution
                      tracker_distribution
                          [itr_dist][max_hypothesis_score_itr_dist_subimg] = 0;
                  ...
                  // Longest lifespan
                    ...
                    auto tracker =
                        haste_trackers[itr_haste].tracker.returnTracker();
                    tracking_duration_time =
                        tracker->t() -
                        haste_trackers[itr_haste].track_start_time;
                    if (max_hypothesis_score != 0 &&
...
int main(int argc, char **argv) {
  ...
  num_distribution = static_cast<unsigned int>(camera.width / subimage_size) *
                     static_cast<unsigned int>(camera.height / subimage_size);
  tracker_distribution =
      (unsigned int **)malloc(sizeof(unsigned int *) * num_distribution);
  base_tracker_distribution = (unsigned int *)malloc(
      sizeof(unsigned int) * num_distribution * max_duplication);
